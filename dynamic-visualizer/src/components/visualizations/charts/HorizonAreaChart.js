...
export default function HorizonAreaChart(props) {
    ...
    const data = response.filter(item => response.filter(x => x.id === item.id).length > 1);  // display only data that is in more then one dataset
    const id = uniqueId();
    ...
    const ref = useD3((svg) => {
        ...
        g.attr("clip-path", (_, i) => `url(${new URL(`#${uid}-clip-${i}`, document.location)})`)
            .selectAll("use")
