...
public class UIMADocument {
   ...
   */
  public UIMADocument(MultipartFile xmlDocument, String group, Integer number, Integer split)
      throws UIMAException {
    this.gson = new Gson();
  ...
  public Map<String, List<UIMATypeMapper>> addPosValues(Map<String, List<UIMATypeMapper>> result) {
    ...
    List<UIMATypeMapper> lemmas = result.get(
        "de_tudarmstadt_ukp_dkpro_core_api_segmentation_type_Lemma");
    ...
    for (Map.Entry<String, List<UIMATypeMapper>> entry : result.entrySet()) {
      ...
      if (entry.getKey().toLowerCase().contains("pos") || entry.getKey().toLowerCase()
          .contains("entity")) {
